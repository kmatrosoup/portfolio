...
/* 非公開変数 */
const float cMissile::m_Radius(5.0f);		// 半径
const float cMissile::m_Accelerator(0.2f);	// 加速度
...
/* コンストラクタ */
cMissile::cMissile(cPlayer* Owner, int AtkDamage, VEC2F Pos, float Angle, int Ricochet)
	: cBaseAtkobj(Owner)
	, m_HitFlag(false)
...
{
	cGame::ShakeEffect(10 + m_AtkDamage * 5);
...
{
	if (m_ExplosionFlag) {
		cAtkobjMgr::GetInstance().Create_Blast(m_Owner, 1, m_Pos, 60.0f + m_AtkDamage * 20.0f);
		cGame::ShakeEffect(20 + m_AtkDamage * 10);
	}
}
...
void cMissile::Update()
{
	...
	for (auto& i : cGame::pVecPlayer)
	{
		...
		if (VEC2F::CheckIntersect_lineCircle(m_Pos_prev, m_Pos, i->Pos, i->Radius + m_Radius)) {
			m_HitFlag = m_ExplosionFlag = true;
			if (!nearest_player) {
	...
	if (m_HitFlag) {
		if (nearest_player->IsShield() && cosf(nearest_player->Angle - m_Angle) < 0.0f) {
			m_HitFlag = m_ExplosionFlag = false;
...
/* 描画・DEFAULT */
void cMissile::Draw_Default() const
{
...
/* 描画・ADD */
void cMissile::Draw_FadeAdd() const
{
	unsigned int color = 0xffffff;
	switch (m_Owner->PlayerID) {
