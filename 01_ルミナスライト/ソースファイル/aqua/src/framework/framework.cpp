...
const unsigned int  aqua::core::CFramework::m_width                     = 1200;
const unsigned int  aqua::core::CFramework::m_height                    = 800;
...
Update( void )
{
    ...
    while( ProcessMessage( ) != AQUA_DX_ERROR)
    {
        ...
        // Escキーで終了
        if( CheckHitKey( KEY_INPUT_ESCAPE ) == TRUE || m_ExitGameFlag )
            break;
    }
}
